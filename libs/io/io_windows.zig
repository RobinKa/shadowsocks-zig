...
fn buffer_limit(buffer_len: usize) usize {
    return std.math.min(std.math.maxInt(isize), buffer_len);
}
...
pub const IO = struct {
    ...
    pub fn init(entries: u12, flags: u32) !@This() {
        _ = entries;
        _ = flags;
            ...
            struct {
                fn do_operation(ctx: Completion.Context, op: anytype) ConnectError!void {
                    ...
                    const rc = blk: {
                        ...
                        var bind_addr: std.net.Address = undefined;
                        switch (op.address.any.family) {
                            std.os.AF.INET => {
                                const inaddr_any = std.mem.zeroes([4]u8);
                                bind_addr = std.net.Address.initIp4(inaddr_any, 0);
                            },
                            std.os.AF.INET6 => {
                                const inaddr_any = std.mem.zeroes([16]u8);
                                bind_addr = std.net.Address.initIp6(inaddr_any, 0, 0, 0);
                            },
                            else => unreachable,
                        }
